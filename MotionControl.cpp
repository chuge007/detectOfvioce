    ...
    , config(NdtCfgMachine::getInstance())
{
    ...
    connect(scanDetectCtrl, &ScanControlAbstract::modbusStateChange,this, &MotionControl::modbusStateChange);
    ...
    connect(ui->writeInPLC_but, &QPushButton::clicked, this, &MotionControl::pbWriteInPLC);
    connect(scanDetectCtrl, &ScanControlAbstract::positionChange, this, &MotionControl::updatePosition);
    ...
    connect(ui->startScan_But, &QPushButton::clicked, this, &MotionControl::pbStartScanBtn);

    connect(ui->lineVelocity_lin, &QLineEdit::editingFinished, this, &MotionControl::PblinVelocity_lin);
    connect(ui->arcVelocity_lin, &QLineEdit::editingFinished, this, &MotionControl::PbarcVelocity_lin);
    connect(ui->AxleVelocity_lin, &QLineEdit::editingFinished, this, &MotionControl::PbAxleVelocity_lin);
    connect(ui->pointVelocity_lin, &QLineEdit::editingFinished, this, &MotionControl::PbPointVelocity_lin);

    connect(ui->setOrigin_but, &QPushButton::clicked, this, &MotionControl::PbSetOrigin);
    connect(ui->smooth_but, &QPushButton::clicked, this, &MotionControl::PbsmoothCurve);
    connect(ui->PoseCalculation_but, &QPushButton::clicked, this, &MotionControl::CalculatingAngles);
    connect(ui->insertArcPos_but, &QPushButton::clicked, this, &MotionControl::pbAddArcPos);
    connect(ui->insertLinePos_but, &QPushButton::clicked, this, &MotionControl::pbAddLinePos);
    connect(ui->DXFimport_but, &QPushButton::clicked, this, &MotionControl::pbDXFimportBut);
    connect(ui->creatG_but, &QPushButton::clicked, this, &MotionControl::PbCreatGcode);
    connect(ui->cleanTable_but, &QPushButton::clicked, this, &MotionControl::cleanTable);
    connect(ui->connect_but, &QPushButton::clicked, this, &MotionControl::PbModbusConnectBtn);
    connect(ui->moveToPosition_but, &QPushButton::clicked, this, &MotionControl::PbMoveToPosition);
    connect(ui->plcType_cb, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &MotionControl::cbSelectPlcType);
    connect(ui->moveUpTabelRow_btu, &QPushButton::clicked, this, &MotionControl::pbmoveUpForSort);
    connect(ui->moveDownTabelRow_btu,&QPushButton::clicked, this, &MotionControl::pbmoveDownForSort);
    connect(ui->setTrajecStart_but,&QPushButton::clicked, this, &MotionControl::pbsetTrajec_start_clicked);
    connect(ui->getCurryPoint_but,&QPushButton::clicked, this, &MotionControl::pbGetModelPoint);
    connect(ui->moveDirectionNot_but,&QPushButton::clicked, this, &MotionControl::pbMoveDirectionNot);
    connect(ui->pBdeletePiece,&QPushButton::clicked, this, &MotionControl::pbdeletePiece);
    connect(ui->pBbrazing,&QPushButton::clicked, this, &MotionControl::pBbrazing);
    connect(ui->imageProcess_but,&QPushButton::clicked, this, &MotionControl::PbImageProcess);
    connect(ui->ascan_but,&QPushButton::clicked, this, &MotionControl::pbAscan);

    connect(ui->processType_cb,QOverload<int>::of(&QComboBox::currentIndexChanged), this, &MotionControl::selectChange);
    connect(ui->cBworkpiece,QOverload<int>::of(&QComboBox::currentIndexChanged), this, &MotionControl::selectChange);

    connect(ui->cBworkpiece,
            QOverload<const QString &>::of(&QComboBox::currentIndexChanged),
            this,
            &MotionControl::selectWorkpiece);
    connect(ui->pbNewPiece,&QPushButton::clicked, this, &MotionControl::pbnewPiece);

    connect(ui->trajectoryOffset_but,&QPushButton::clicked, this, &MotionControl::PbtrajectoryOffset);
    ...
    connect(scene, &QGraphicsScene::selectionChanged,
            this, &MotionControl::graphicsSelectionChanged);
...
void MotionControl::closeEvent(QCloseEvent *event)
{
...
void MotionControl::updatePosition(QPointF pos,float cur_r,float cur_z)
{
...
void MotionControl::pbAddSpline()
{   qDebug()<<"pbAddSpline";
...
void MotionControl::pbAddElliptical()
{
...
void MotionControl::pbAddArcPos()
{
    ...
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 图元已添加   "));


}

void MotionControl::pbAddLinePos()
{
    //    if(!scanDetectCtrl->modbusState()){
    //        QMessageBox::warning(nullptr, "error",QString::fromLocal8Bit( " 通讯未连接   "));
    //        return;
...
void MotionControl::on_editPos_but_clicked()
{
...
void MotionControl::on_delete_but_clicked()
{
    ...
    std::sort(rows.begin(), rows.end(), std::greater<int>());  // 降序删除行，避免影响后面行号

    int ret = QMessageBox::warning(this, QStringLiteral("删除选中行"), QStringLiteral("确定删除选中行吗？"), QMessageBox::Yes | QMessageBox::No);
    if (ret == QMessageBox::Yes) {
        for (int row : rows) {
            model->removeRow(row);  // 直接调用QSqlTableModel的removeRow
        }
        if (!model->submitAll()) {
        ...
        model->revertAll();
        dbManager->db.commit();  // 不删除也要提交事务结束
    }
...
void MotionControl::pbWriteInPLC()
{
...
void MotionControl::pbStartScanBtn(){
...
void MotionControl::updateSence()//on_testRout_but_clicked()
{
...
void MotionControl::tableSelectionChanged()
{
...
void MotionControl::graphicsSelectionChanged()
{
...
void MotionControl::pbDXFimportBut()
{
...
void MotionControl::sortModelLine()
{
    qDebug() << "开始排序模型图形链...";
    dbManager->db.transaction();
...
void MotionControl::pbsetTrajec_start_clicked()
{
    // 开始数据库事务
    dbManager->db.transaction();
    ...
    // 更新场景视图
    updateSence();
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 起点已添加   "));

}


QString MotionControl::generateGCode(/*const QVector<TrackSegment>& segments*/)
{
    ...
    for(int i = 0; i < row_count; i++)
    {
        ...
        if(name=="line"){
            ...
            if(i!=0){
                gCode += QString("(N%1 G01 XF%2 YF%3 ZF%4 XS%5 YS%6 ZS%7 A%8)\n")
                        .arg(firstNum+=10)
                        .arg(x0)
                        .arg(y0)
                        .arg(z0)
    ...
    //        r2 += sign * step;
    //        for (float a = r2; std::abs(a) > 90; a += sign * step) {
    //            gCode += QString("N%1 G00 X0 Y0 Z0 A%2 E50 F30 F-30\n")
    //                         .arg(firstNum += 10 )
    //                         .arg(QString::number(a, 'f', 1));

    //            qDebug()<<"gCode:"<<QString("N%1 G01 X0 Y0 Z0 A%2\n")
    //                      .arg(firstNum  )
    //                      .arg(QString::number(a, 'f', 1));

    //            gCode+=QString("N%1 G04 P0.5\n").arg(firstNum += 10 );
    //            count++;
...
//void MotionControl::CalculatingAngles(){
...
//                                  QString::fromLocal8Bit("是否校准当前点为第一个点，并把其R值作为角度计算的基点？（否则以0作为基点）"),
//                                  QMessageBox::Yes | QMessageBox::No);
...
//       inputR=ui->rCurPos_lab->text().toDouble();
...
//                 qDebug()<<"angleDegbefor  <0 "<<i;
//            }

//            float DifferenceA =fabs(angleDegEquivalence-inputR);
//            float DifferenceB =fabs(angleDeg-inputR);


//            qDebug()<<"curryR"<<inputR<<"   cycleNum:"<<cycleNum;
//            qDebug()<<"angleDegEquivalence: "<<angleDegEquivalence<<"   "<<i;
...
//            inputR = (DifferenceA < DifferenceB) ? angleDegEquivalence : angleDeg;
...
//            model->setData(model->index(i, 13), std::round(inputR * 100.0) / 100.0);


//            //qDebug()<<"line: "<<inputR<<"   "<<i;
...
//            // 将弧度转为角度（如果你希望以角度为单位加到 inputR 上）
...
void MotionControl::CalculatingAngles(){
    ...
    double inputR,startR;

    if (reply == QMessageBox::Yes){


        startR=ui->rCurPos_lab->text().toDouble();
    ...
    for(int i = 0; i < row_count; i++)
    {
        ...
        // 调整为与 inputR 最接近的角度
        auto adjustAngleContinuity = [](float angleDeg, float inputR) -> float {
            float delta = angleDeg - inputR;
            qDebug() << "adjustAngleContinuity angleDeg: " << angleDeg ;
            qDebug() << "delta: " << delta ;
            qDebug() << "inputR: " << inputR ;
            return angleDeg - std::round(delta / 360.0f) * 360.0f;
        };
        ...
        if(name=="line"){
            ...
            angleDeg = adjustAngleContinuity(angleDeg, inputR);

            qDebug() << "Adjusted angleDeg: " << angleDeg << " i=" << i;

            inputR = angleDeg+startR;
...
void MotionControl::PbCreatGcode()
{
    if(ui->cBworkpiece->currentText()==""){
        QMessageBox::warning(nullptr, "error", QString::fromLocal8Bit("请选择工件"));
        return;
    ...
    gcodeEidt->filePath=gcodeEidt->gcodePath+QString("%1.cnc").arg(gcodeEidt->workPiece);

    qDebug()<<"gcode filePath :"<<gcodeEidt->filePath;
    gcodeEidt->show();
    QTimer::singleShot(0, gcodeEidt, [this](){
        gcodeEidt->loadGCodeToPlainText();
    });
    ui->disMess_lab->setText(QString::fromLocal8Bit(" g代码已导出   "));

}


void MotionControl::cleanTable(){
...
void MotionControl::PbAxleVelocity_lin(){
    ...
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 定位速度%1已设置   ").arg(ui->AxleVelocity_lin->text()));
    saveSetting();
}



void MotionControl::PbPointVelocity_lin(){
    ...
    //QMessageBox::warning(nullptr, "error", QString::fromLocal8Bit(" 速度已设置   "));
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 点动速度%1已设置   ").arg(ui->pointVelocity_lin->text()));
    saveSetting();
}


void MotionControl::PblinVelocity_lin(){
    ...
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 直线速度%1已设置   ").arg(ui->lineVelocity_lin->text()));
    saveSetting();

}

void MotionControl::PbarcVelocity_lin(){
    ...
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 圆弧速度%1已设置   ").arg(ui->arcVelocity_lin->text()));
    saveSetting();

}
void MotionControl::PbtrajectoryOffset() {
    bool ok;
    ...
    settings->setValue("originR", originR);

    ui->disMess_lab->setText(QString::fromLocal8Bit("当前角度:%1").arg(originR));
    model->submitAll();
    model->select();
    updateSence();
    if (!dbManager->db.commit()) {
    ...
    ui->disMess_lab->setText(QString::fromLocal8Bit(" 已偏移 x%1 y%2 r%3  ").arg(_translationX).arg(_translationY).arg(_translationR));
...
void MotionControl::PbImageProcess()
{
...
void MotionControl::modbusStateChange(QModbusDevice::State state){
...
void MotionControl::insertSmoothArcBetween(int id,int prevRow, int nextRow, qreal smoothFactor)
{
    dbManager->db.transaction();

    mathTool mathTool;
    if (prevRow < 0 || nextRow >= model->rowCount()) return;

    ArcInsertResult result;
    QString prevRowTy=model->data(model->index(prevRow, 1)).toString();
    QString nextRowTy=model->data(model->index(nextRow, 1)).toString();
    if(prevRowTy=="line"&&nextRowTy=="line"){
        // 取出前后点和方向向量
...
void MotionControl::selectWorkpiece(){
...
void MotionControl::selectChange(){
...
void MotionControl::cbSelectPlcType(int index){
...
// 上移按钮点击事件
void MotionControl::pbmoveUpForSort()
{
...
// 下移按钮点击事件
void MotionControl::pbmoveDownForSort()
{
...
void MotionControl::pbMoveDirectionNot(){
...
void MotionControl::pbGetModelPoint(){
...
std::tuple<double, double, double, double> MotionControl::pbGetCurrentlyPoint(){
...
void MotionControl::createOrSwitchTable(const QString &tableName, bool isCreate)
{
    ...
    if (isCreate) {
        ...
        if (tableName == currentTable) {
            qDebug() << "The target table is the same as the current table, skip copying.。";
            return;
        }
        ...
        if (q.next()) {
            ...
            QString clearSQL = QString("DELETE FROM %1;").arg(qtgt);
            if (!q.exec(clearSQL)) {
                qDebug() << "清空目标表失败：" << q.lastError().text() << clearSQL;
                dbManager->db.rollback();
                return;
            }
...
void MotionControl::saveSetting() {
...
void MotionControl::initSetting() {
...
void MotionControl::resizeEvent(QResizeEvent *event)
{
    ...
    //    //        //QMotionControl::resizeEvent(event);
    //    //        return;
    //    //    }
...
void MotionControl::scaleWidgets(QWidget* parent, double scaleX, double scaleY)
{
...
void MotionControl::restoreOriginalWidgetStates(QWidget* parent)
{
    for (QObject* obj : parent->children()) {
        QWidget* w = qobject_cast<QWidget*>(obj);
        if (!w || w->inherits("QLayout"))
...
void MotionControl::saveOriginalWidgetStates(QWidget* parent)
{
    oldSize=this->size();
    for (QObject* obj : parent->children()) {
        QWidget* w = qobject_cast<QWidget*>(obj);
...
//    int i = 0;
//    QEventLoop loop;
//    connect(this, &MotionControl::targetReached, &loop, &QEventLoop::quit);
...
//QList<QByteArray> MotionControl::splitByFour(const QByteArray &data) {
...
//    return result;
//}
//QList<int> MotionControl::convertToDecimal(const QByteArray &data) {
//    QList<int> result;
//    for (char byte : data) {
...
//    return result;
//}
//QList<float> MotionControl::convertToReal(const QByteArray &data) {
//    QList<float> result;
...
//void MotionControl::pbIpLineEdit(const QString &arg1)
//{
//    qDebug()<<"on_ipLineEdit_textChanged";
//    scanDetectCtrl->setModbusTcpIP(arg1);
//}
//void MotionControl::readPendingDatagrams()
//{
...
//*************************************************************8
//QByteArray MotionControl::createModbusRequest(int registerAddress)
...
//    return request;
//}
//void MotionControl::onStateChanged(QModbusDevice::State state)
//{
...
//            if (!reply->isFinished())
//                connect(reply, &QModbusReply::finished, this, &MotionControl::readReady);
//            else
//                delete reply;
//        } else {
...
//void MotionControl::onModbusError(QModbusDevice::Error error)
...
//float MotionControl::readModbusFloatData(int v1, int v2)
...
//void MotionControl::pbModbusConnect()
...
//connect(udpSocket, &QUdpSocket::readyRead, this, &MotionControl::readPendingDatagrams);

//connect(ui->connectButton, &QPushButton::clicked, this, &MotionControl::pbModbusConnect);
//connect(modbusDevice, &QModbusTcpClient::stateChanged, this, &MotionControl::onStateChanged);
//connect(modbusDevice, &QModbusTcpClient::errorOccurred, this, &MotionControl::onModbusError);
...
//QString MotionControl::parseModbusResponse(const QByteArray &response)
//{
//    if (response.size() < 9) {
