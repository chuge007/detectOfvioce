...
//    currentPos = QPointF(QRandomGenerator::global()->bounded(9999.99), QRandomGenerator::global()->bounded(9999.99));
    ...
    for (auto &segment : dxfReader.trackSegments) {
        QVector<QPointF> path_points;
        qDebug()<<"***trackSegments="<<dxfReader.trackSegments.count();
        if (segment.type == "Line") {
            //std::cout << "Detected Line Segment" << std::endl;
            QPointF start_pos(segment.points.at(0),segment.points.at(1));
            QPointF end_pos(segment.points.at(2),segment.points.at(3));
            path_points.append(start_pos);
            path_points.append(end_pos);
            //type_lst.append("line");
            //qDebug()<<"***start_pos="<<start_pos<<"end_pos"<<end_pos;
            dxfPaths_LA.append(path_points);
        } else if (segment.type == "Arc") {
            //std::cout << "Detected Arc Segment" << std::endl;
            double cx =segment.points.at(0);
            double cy =segment.points.at(1);
            double radius =segment.points.at(2);
            double angle1 =segment.points.at(3);
            double angle2 =segment.points.at(4);

            double startXPos=0,startYPos=0,endXPos=0,endYPos=0;
            startXPos = cx + cos(abs(angle1)*M_PI/180)*radius;
            startYPos = cy + sin(abs(angle1)*M_PI/180)*radius;
            endXPos = cx + cos(abs(angle2)*M_PI/180)*radius;
            endYPos = cy + sin(abs(angle2)*M_PI/180)*radius;
            QPointF startPoint=QPointF(0,0);
            QPointF endPoint=QPointF(0,0);
            QPointF transPoint=QPointF(0,0);

            //画圆弧始终逆时针转 angle1>angle2 当起始角大于终止角时定义的是逆时针的圆弧
            if(angle1>angle2){
                startPoint= QPointF(endXPos,endYPos);
                endPoint=QPointF(startXPos,startYPos);
                double tranAngle = angle2 +(angle1+360-angle2)/2;
                double transXPos = cx + cos(abs(tranAngle)*M_PI/180)*radius;
                double transYPos = cy + sin(abs(tranAngle)*M_PI/180)*radius;
                transPoint=QPointF(transXPos,transYPos);
            }
            else{
                startPoint= QPointF(startXPos,startYPos);
                endPoint=QPointF(endXPos,endYPos);
                double tranAngle = angle1 +(angle2-angle1)/2;
                double transXPos = cx + cos(abs(tranAngle)*M_PI/180)*radius;
                double transYPos = cy + sin(abs(tranAngle)*M_PI/180)*radius;
                transPoint=QPointF(transXPos,transYPos);
            }
...
//        double mz = d.mz;
        ...
        startXPos=c_x+R*cos(a);
        startYPos=c_y+R*sin(a);
        path.moveTo(startXPos, startYPos);
