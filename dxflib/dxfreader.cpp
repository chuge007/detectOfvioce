...
void DxfReader::addLine(const DL_LineData &data)
{
    ...
    segment.type = "Line";
    segment.points = {data.x1, data.y1, data.x2, data.y2};
    trackSegments.push_back(segment);

    std::cout << "Line: Start(" << data.x1 << ", " << data.y1
                      << ") End(" << data.x2 << ", " << data.y2 << ")" << std::endl;
}
...
void DxfReader::addArc(const DL_ArcData &data)
{
    ...
    TrackSegment segment;
    segment.type = "Arc";
    segment.points = {data.cx, data.cy, data.radius, data.angle1, data.angle2};
    trackSegments.push_back(segment);

    std::cout << "Arc: Center(" << data.cx << ", " << data.cy
                      << ") Radius: " << data.radius << " Start Angle: " << data.angle1
                      << " End Angle: " << data.angle2 << std::endl;
}
